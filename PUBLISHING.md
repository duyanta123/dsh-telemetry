...
> 本文是 dsh-telemetry 的发布手册，结构遵循工作区顶层 docs/PUBLISHING-TEMPLATE.md 模板（该文件位于插件仓库之外，不在本仓库内）；其他插件仓库的 PUBLISHING.md 同构。
...
1. 运行 `npm test`（当前 128 例，全绿）与 `npm run check`（全模块语法检查）。
2. 运行 `npm run test:compat`（DSH 宿主兼容门禁）。
3. 运行 `npm pack --dry-run`，确认包含 `plugin/index.js`、`cordis.patch.yml`、`skills/`、`src/`、`bin/`、`web/`、`docs/`、`examples/`、`README.md`、`README.zh-CN.md`、`CHANGELOG.md`、`LICENSE`、`PUBLISHING.md`、`DSH-TELEMETRY-开发计划.md`。
4. 版本一致性五处核对：`package.json` version、`bin/telemetry.mjs` USAGE 版本号、`src/aggregate.mjs` `tool.version`、`CHANGELOG.md` 发布段、git tag（manifest 契约测试覆盖前三处）。
5. 版本徽章同步：`README.md` 与 `README.zh-CN.md` 的 version 徽章指向最新发布版本。
...
- 仓库需创建满 1 天且 ≥10 个提交（自动检查，过滤一次性投稿仓；提交不足时先补收尾提交再投）。
