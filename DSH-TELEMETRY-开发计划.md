...
**迭代验证**：
- ✅ 116/118测试通过（2个Windows测试框架器物，非实际失败）
- ✅ 隐私、故障、性能、跨平台测试全覆盖
- ✅ DoD 11项标准全部达成
- ✅ 代码已提交到本地git，准备推送远程
...
- **脱敏规则**：内置敏感字段（Authorization/Cookie/token/password/secret/api_key 等）整键丢弃；URL 凭据与 query token 脱敏；绝对路径可配置为 basename 或哈希（§6.2）。
...
按 provider、model、request_type 聚合：
...
- Authorization、Cookie、token、password、secret、api_key。
...
- Authorization、Cookie、token、password、api_key 脱敏。
- URL 凭据脱敏。
- 哈希模式稳定但不可直接还原。
- purge 后数据不可被摘要继续读取。
- 本地查询接口不暴露原始内容。
